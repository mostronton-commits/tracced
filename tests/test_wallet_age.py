...
class TestWalletAge(unittest.TestCase):
    ...
    def test_bound_only_at_limit(self):
        wa = WalletAge(post=FakePost([sigs(1000)]), sleep=lambda s: None, pace_s=0)
        a = wa.oldest_tx("W2")
        self.assertFalse(a["exact"])
        self.assertEqual(a["n"], 1000)
    ...
    def test_pacing_and_cache(self):
        ...
        self.assertEqual(len(post.calls), 2)                              # third call — from cache
