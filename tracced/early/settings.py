"""Налаштування early: дефолти в коді + блок `early:` з config.yaml поверх."""
import copy

from ..config import _deep_merge

DEFAULTS = {
    "scopes": [24, 48],          # масштаби цифр на сторінці: уся історія + стільки годин після діапазону
    # тариф Solana Tracker задає стелі (PLAN_CAPS); явні значення в config.yaml сильніші за тариф
    "plan": "free",
    "max_trade_pages": 300,      # стеля сторінок /trades за один прогін (500 угод/стор.)
    # вікно входу тягнемо завжди повністю (до стількох сторінок); далі рішення budget.choose_mode:
    # повний діапазон або угоди кожного гаманця (до стількох гаманців)
    "max_window_pages": 60,
    "max_wallet_lookups": 500,
    "budget_guard_pct": 25,      # один прогін ≤ стільки % залишку кредитів (0 = вимкнено)
    # місяць: скільки кредитів дає тариф і скільки лишити в резерві. Коли залишок мінус найгірший прогін падає нижче
    # резерву, нові живі аналізи не-адмінів чекають оновлення тарифу (0 = не перевіряти)
    "credits_month": 0,
    "credits_reserve_pct": 0,
    "full_fetch_margin": 1.15,   # запас до оцінки сторінок повного діапазону
    "pause_s": 0.35,             # пауза між запитами до ST (free: 3 req/s)
    # скільки запитів до ST іде одночасно: угоди різних гаманців незалежні, тож їх тягнемо паралельно.
    # free тримає темп 3 req/s, і паралельність там нічого не дає; Pro обмеження швидкості не має
    "st_concurrency": 1,
    # людські ліміти (показуються як повідомлення, не як «сторінки»)
    "max_window_hours": 24,      # вікно входу не довше
    "wallet_stats_ttl_hours": 24,
    "max_wallet_trade_pages": 4,  # угоди одного гаманця по токену: сторінок по 500 (боти)
    # тег `fresh`: вік гаманця з публічного RPC у фоні після аналізу (0 = вимкнено)
    "age_lookups_max": 300,
    "age_full_top": 200,         # з них перші за PnL — повна перевірка (вік зайнятого гаманця, спонсор застосунку); решта дешева
    "age_lookups_public": 200,   # без власної ноди (лише публічна api.mainnet-beta, що ріже серії) — перші 200 за PnL
    "rpc_pace_s": 0.5,           # пауза між запитами до публічної ноди (ріже серії без пауз)
    # пауза між запитами транзакцій (спонсор гаманця): вони йдуть у публічну api.mainnet-beta, навіть коли підписи
    # питаємо в платній ноді, а публічна тримає ~40 викликів за 10 с на метод
    "rpc_tx_pace_s": 0.3,
    # платна RPC-нода (SOLANA_RPC_URL) — окремий продукт зі своїм пулом кредитів: 10 за кожен виклик підписів.
    # Збагачення стає на паузу, коли за місяць витрачено все, крім резерву. 500 000 = безкоштовний тариф RPC
    "rpc_credits_month": 500000,
    "rpc_reserve_pct": 10,
    "wallet_age_ttl_hours": 168,
    "markers_max": 200,          # мітки угод одного гаманця на графіку
    # картка гаманця: його угоди по всіх токенах за останні дні, порахунок нашим леджером (не чужий PnL)
    "profile_days": 30,
    "profile_max_pages": 5,      # до 1000 обмінів на сторінку; активнішим гаманцям показуємо «partial»
    "wallet_profile_ttl_hours": 24,
    # хто стоїть за гаманцем (KOL, Twitter, платформа): Solana Tracker додає це до сторінок угод на прохання
    "st_identity": True,
    "identity_ttl_hours": 720,
    "example_job": None,         # id аналізу, закріпленого на головній як приклад (None = найстарший готовий)
    "demo_job": None,            # id аналізу-демо: його токен і діапазон програються зі знімка без запитів до ST
    "replay_s": 12,              # скільки секунд триває програвання демо-аналізу в терміналі
    "runs_per_day": 1,           # живих аналізів на добу на людину: гаманець і браузер рахуються разом (адмін — без ліміту)
    "runs_per_ip_per_day": 10,   # живих аналізів на добу з однієї мережі: інкогніто з новим гаманцем упирається сюди
    "refund_below_requests": 50, # прогін, що впав, повертає спробу, лише якщо встиг витратити менше запитів
    "age_card_per_day": 50,      # платних перевірок віку/спонсора з карток на людину за добу (з кешу — безкоштовно)
    "age_card_global_per_day": 500,   # і на весь сайт за добу: одна перевірка — до ~70 кредитів RPC
    "demo_replays_per_10min": 10,     # програвань демо з однієї адреси за 10 хвилин
    "runs_per_hour": 20,         # стартів живих аналізів з однієї адреси за годину; наступний чекає годину
    "runs_global_per_day": 10,   # живих аналізів на весь сайт за добу (адмін поза стелею): будь-який ключ підписує безкоштовно
    "ranges_per_token": 3,       # діапазонів (і аналізів) на один токен; видалив свій — місце вільне
    "run_cap_requests": 2000,    # запитів до Solana Tracker на один прогін не-адміна (адмін — без стелі)
    "finder_pumps": 2,           # скільки пампів пропонує «Find the pump»
    # графік живого токена коштує запитів (огляд ≈2, шматок свічок 1), а дивитись його може будь-хто: добові стелі
    "browse_per_day_guest": 30,  # …з однієї адреси без гаманця
    "browse_per_day": 150,       # …з гаманця
    "browse_global_per_day": 300,   # …на весь сайт (адміни поза стелею; кеш безкоштовний)
    "agent_questions_per_day": 10,  # питань до AI-агента на добу з гаманця (гостям агент закритий)
    "agent_cards_per_day": 20,      # нових трьох карток на добу з гаманця (готові — безкоштовно для всіх)
    "agent_global_per_day": 300,    # спільна добова стеля агента на весь сайт (одна відповідь ≈ $0.001)
    # дашборд власника: що роблять підключені гаманці і скільки це коштує (output/early/usage)
    "usage_tz": "UTC",              # доба дашборда (config.yaml: Europe/Warsaw); добові стелі сайту однаково за UTC
    "usage_events_per_day": 1000,   # рядків переглядів і кліків на гаманець за добу: більше диску не віддаємо
    "usage_events_global_per_day": 30000,   # і на весь сайт
    "usage_keep_months": 13,        # скільки місяців журналу тримати
    "usage_active_days": 7,         # on-chain факти оновлюються для тих, хто був активний за стільки днів
    "usage_onchain_max_wallets": 100,   # гаманців за один прохід оновлення
    "usage_profiles_per_day": 50,   # нових 30-денних профілів за добу (1-5 запитів кожен; кеш картки спільний)
    "feedback_per_day": 200,        # листів «Write to us» на весь сайт за добу (з однієї адреси — 5 на годину)
    "min_invested_usd": 20,      # пил: хто вклав менше — у таблицю не потрапляє
    "page_size": 500,            # для оцінки ціни прогону; стільки ж просимо в /trades (стеля API)
    # ціна повного шляху: темп угод у пампі не тримається далі, тому дірку МІРЯЄМО кількома запитами
    "probe_pages": 3,            # скільки сторінок-проб (0 = не міряти, вірити екстраполяції)
    "probe_min_pages": 200,      # міряти, лише коли груба оцінка така велика, що рішення дороге
    # вибір інтервалу свічок за довжиною відрізка (годин); довше — 1h
    "chart_span_hours": {"1m": 6, "5m": 48, "15m": 24 * 14},
    "suggest_peak_hours": 6,     # пік пампу шукаємо стільки годин після старту
}


PLAN_CAPS = {
    "free":     {"max_trade_pages": 300, "max_wallet_lookups": 500, "max_window_pages": 60,
                 "budget_guard_pct": 25, "pause_s": 0.35, "st_concurrency": 1, "credits_month": 2500},
    # pause_s 0: спільний темп між потоками інакше тримав би стелю 1/pause запитів на секунду, скільки б
    # потоків не було. Швидкість тримає st_concurrency, а не пауза
    "advanced": {"max_trade_pages": 2000, "max_wallet_lookups": 3000, "max_window_pages": 400,
                 "budget_guard_pct": 0, "pause_s": 0.0, "st_concurrency": 8,
                 "credits_month": 1_000_000, "credits_reserve_pct": 5},
}


def load(cfg=None):
    """дефолти → стелі тарифу → явні значення з config.yaml (сильніші за тариф)."""
    over = (cfg or {}).get("early") or {}
    plan = over.get("plan", DEFAULTS["plan"])
    if plan not in PLAN_CAPS:
        raise ValueError(f"early.plan must be one of {sorted(PLAN_CAPS)}, got {plan!r}")
    s = copy.deepcopy(DEFAULTS)
    _deep_merge(s, PLAN_CAPS[plan])
    _deep_merge(s, over)
    return s


def chart_interval(span_ms, s):
    """Найдрібніший інтервал, при якому свічок лишається розумна кількість."""
    hours = span_ms / 3_600_000
    for itv in ("1m", "5m", "15m"):
        lim = (s.get("chart_span_hours") or {}).get(itv)
        if lim and hours <= lim:
            return itv
    return "1h"


INTERVAL_MS = {"1m": 60_000, "5m": 300_000, "15m": 900_000, "1h": 3_600_000}
