...
(`ensure(..., workers=K)`): кожна прогалина стартує одним ланцюжком сторінок, густий шматок по дорозі ділиться навпіл.
...
class TradeStore:
    ...
    def _ensure_parallel(self, a, b, fetch_page, max_pages, log, pages_done, on_page, workers):
        """Кожна прогалина стартує одним ланцюжком сторінок від свого початку, як у послідовному шляху. Густина угод
        нерівна (памп — хвилини, решта — дні), тож шматок, у якому після сторінки лишається багато сторінок,
        ділиться навпіл, щойно якийсь потік простоює; так густа історія розходиться на всі потоки за кілька сторінок.
        Різати наперед на `workers` шматків не можна: кожен шматок коштує щонайменше сторінку, і тиха історія,
        яку покриває одна-дві сторінки, коштувала б вісім.
        ...
        todo = deque()
        for ga, gb in self.gaps(a, b):
            todo.append((ga, gb, ga - 1))                              # (початок, кінець, курсор)
        pending = {}
